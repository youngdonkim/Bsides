...
- **없음** (정적 사이트·외부 link redirect만, 예: Bsides v1) → 적용 0
...
| 사이클 라벨 | 강도 |
|---|---|
| `v*-prototype` | 측정·기록만 (위반은 retro 기록, 차단 X) |
| `v*-mvp` | 경고 + 사용자 선택 |
| `v*-prod` | 강제 차단 |
...
영역별 상세 룰은 `.claude/rules/` 다른 파일 (`sensitive-data-exposure`·`page-auth-pattern`·`file-upload-security`·`kakao-auth-share` 등) 참조.
